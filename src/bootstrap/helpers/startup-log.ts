...
export function logStartup(app: INestApplication, host: string, port: number, prefix?: string) {
  const normPrefix = String(prefix ?? '').replace(/^\/+|\/+$/g, '');
  const baseUrl = normPrefix ? `http://${host}:${port}/${normPrefix}` : `http://${host}:${port}`;
