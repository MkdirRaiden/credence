...
describe('resolveAndRegister', () => {
  ...
  it('throws error if critical provider not found', () => {
    ...
    expect(() =>
      ...
      `CRITICAL: Provider ${CRITICAL_PROVIDERS[0]} not found for registration`,
    );
  });
  ...
  it('handles multiple providers', () => {
    ...
    expect(moduleRef.get).toHaveBeenNthCalledWith(1, Provider1, {
      strict: false,
    });
    expect(moduleRef.get).toHaveBeenNthCalledWith(2, Provider2, {
      strict: false,
    });
